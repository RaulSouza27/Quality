...
# ... (seu código existente para as outras classes e funções) ...
...
class PainelNCApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Painel de Não Conformidades")
        # Assegura que cfg e ncs sejam carregados antes de criar frames
        self.cfg = self.carregar_cfg_wrapper()
        self.ncs = self.carregar_ncs_wrapper()
        self.frames = {}
        self.topbars = {}
        self.criar_frames()
        self.mostrar_frame("menu")

    def carregar_cfg_wrapper(self):
        # Implementação para carregar configurações (similar à sua original)
        if os.path.exists("config.json"):
            with open("config.json", "r", encoding="utf-8") as f:
                return json.load(f)
        else:
            default_cfg = { # Use uma versão simplificada ou importe a sua DEFAULT_CFG
                "unidades": ["KG", "L"],
                "defeitos": ["Cor", "Cheiro"],
                "cores_defeitos": {},
                "produtos": []
    ...
    def carregar_ncs_wrapper(self):
        # Implementação para carregar NCs (similar à sua original)
        if os.path.exists("ncs.json"):
            with open("ncs.json", "r", encoding="utf-8") as f:
                return json.load(f)
        return []
    ...
    def criar_frames(self):
        self.frames["menu"] = self.criar_menu()
        self.frames["painel"] = tk.Frame(self.root) # Placeholder
        self.frames["config"] = tk.Frame(self.root) # Placeholder
        self.frames["sobre"] = self.criar_sobre()
    ...
    # Adicione as funções de criação de painel e configuração para que o código seja executável
    def criar_painel_nc(self):
        frame = tk.Frame(self.root)
        tk.Label(frame, text="PAINEL DE NÃO CONFORMIDADES").pack()
        return frame

    def criar_configuracoes(self):
        frame = tk.Frame(self.root)
        tk.Label(frame, text="CONFIGURAÇÕES").pack()
        return frame
    ...
    def criar_sobre(self):
        ...
        # Texto a ser exibido na seção "Sobre"
        texto_sobre = f"""
Este aplicativo é uma ferramenta completa e intuitiva, projetada para simplificar o registro, acompanhamento e análise das Não Conformidades (NCs) na sua operação.

Com ele, você pode:

* Cadastrar e Gerenciar NCs: Registre detalhadamente cada ocorrência, desde a data e o produto envolvidos até o tipo de defeito e a ação imediata.
* Visualizar de Forma Clara: Todas as NCs são apresentadas em uma tabela organizada, facilitando a consulta e o acompanhamento do status de cada uma.
* Analisar com Gráficos: Obtenha insights rápidos sobre os principais defeitos e o status geral das NCs através de gráficos visuais.
* Personalizar o Sistema: Adapte unidades de medida, tipos de defeitos e produtos para que o sistema se encaixe perfeitamente às suas necessidades.

Desenvolvido para oferecer agilidade e controle, este gerenciador de NCs é essencial para otimizar seus processos de qualidade.
...
{datetime.date.today().year}
        ...
        # Usando um Text widget para exibir o texto formatado
        text_widget = tk.Text(frame, font=("Arial", 12), wrap="word", height=20, width=80, padx=10, pady=10, relief="flat", bg=frame.cget('bg'))
        text_widget.insert("1.0", texto_sobre)

        # Configurações de tags para negrito (ajuste conforme necessário)
        text_widget.tag_configure("bold", font=("Arial", 12, "bold"))
        
        # Aplicando negrito a palavras-chave
        # Note: As posições das tags (linha.coluna) são sensíveis ao texto.
        # Se você alterar o texto, esses números precisarão ser reajustados.
        text_widget.tag_add("bold", "3.80", "3.86") # simplificar
        text_widget.tag_add("bold", "3.116", "3.123") # análise

        text_widget.tag_add("bold", "7.2", "7.28") # Cadastrar e Gerenciar NCs
        text_widget.tag_add("bold", "8.2", "8.25") # Visualizar de Forma Clara
        text_widget.tag_add("bold", "9.2", "9.20") # Analisar com Gráficos
        text_widget.tag_add("bold", "10.2", "10.24") # Personalizar o Sistema

        text_widget.tag_add("bold", "12.24", "12.31") # agilidade
        text_widget.tag_add("bold", "12.36", "12.44") # controle
...
# O restante do seu código (main execution block)
